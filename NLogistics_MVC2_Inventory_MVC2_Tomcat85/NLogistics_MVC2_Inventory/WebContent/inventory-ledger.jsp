...
<%
    java.util.List<model.InventoryLedger> items = (java.util.List<model.InventoryLedger>) request.getAttribute("items");
    if (items == null) {
        response.sendRedirect("InventoryLedgerController");
        return;
    }
%>
...
<% for(model.InventoryLedger x:items){ %><tr><td><%=x.getLedgerId()%></td><td><%=x.getProductName()%></td><td><span class="badge"><%=x.getTransactionType()%></span></td><td><%=x.getQuantity()%></td><td>&#8377; <%=x.getUnitCostAtTxn()%></td><td><%=x.getReferenceType()%> / <%=x.getReferenceId()%></td><td><%=x.getTxnDate()%></td><td><a class="btn btn-secondary btn-sm" href="InventoryLedgerController?action=edit&id=<%=x.getLedgerId()%>">Edit</a> <a class="btn btn-danger btn-sm" href="InventoryLedgerController?action=delete&id=<%=x.getLedgerId()%>" onclick="return confirm('Delete this ledger entry?')">Delete</a></td></tr><% } %></table></div></div>
</main></body></html>