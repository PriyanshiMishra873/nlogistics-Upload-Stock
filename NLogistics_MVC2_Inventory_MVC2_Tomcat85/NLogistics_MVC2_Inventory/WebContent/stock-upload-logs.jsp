...
<%
    java.util.List<model.StockUploadLog> items = (java.util.List<model.StockUploadLog>) request.getAttribute("items");
    if (items == null) {
        response.sendRedirect("StockUploadLogController");
        return;
    }
%>
...
<% for(model.StockUploadLog x:items){ %><tr><td><%=x.getUploadId()%></td><td><%=x.getCompanyName()%></td><td><%=x.getUploadedByName()%></td><td><%=x.getFileName()%></td><td><%=x.getTotalRecords()%></td><td><%=x.getSuccessCount()%></td><td><%=x.getFailureCount()%></td><td><%=x.getErrorReportPath()%></td><td><%=x.getUploadedAt()%></td><td><a class="btn btn-secondary btn-sm" href="StockUploadLogController?action=edit&id=<%=x.getUploadId()%>">Edit</a> <a class="btn btn-danger btn-sm" href="StockUploadLogController?action=delete&id=<%=x.getUploadId()%>" onclick="return confirm('Delete this upload log?')">Delete</a></td></tr><% } %></table></div></div>
</main></body></html>