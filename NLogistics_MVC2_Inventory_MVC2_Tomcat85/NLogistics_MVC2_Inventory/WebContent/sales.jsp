...
<%
    java.util.List<model.SalesTransaction> items = (java.util.List<model.SalesTransaction>) request.getAttribute("items");
    if (items == null) {
        response.sendRedirect("SalesTransactionController");
        return;
    }
%>
...
<% for(model.SalesTransaction x:items){ %><tr><td><%=x.getTransactionId()%></td><td><%=x.getProductName()%></td><td><%=x.getCustomerName()%></td><td><%=x.getShipmentId()==null?"-":x.getShipmentId()%></td><td><%=x.getQuantitySold()%></td><td>&#8377; <%=x.getSalePriceSnapshot()%></td><td><b>&#8377; <%=x.getSaleAmount()%></b></td><td><%=x.getSaleDate()%></td><td><a class="btn btn-secondary btn-sm" href="SalesTransactionController?action=edit&id=<%=x.getTransactionId()%>">Edit</a> <a class="btn btn-danger btn-sm" href="SalesTransactionController?action=delete&id=<%=x.getTransactionId()%>" onclick="return confirm('Delete this sale?')">Delete</a></td></tr><% } %></table></div></div>
</main></body></html>